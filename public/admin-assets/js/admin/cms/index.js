var CmsList = function () {
	...
	var table = document.getElementById('table_cms');
	var datatable;
	var toolbarBase;
	var toolbarSelected;
	var selectedCount;
				...
				},
				{
					...
					render: function (data, type, full, meta) {
						return '<div class="d-flex flex-column">\
							<a href="'+ full.show_url + '" target="_blank" class="text-gray-800 text-hover-primary mb-1 "><span class="fw-bolder fs-4">' + full.name + '</span></a>\
						</div>';
					},
				},
				{
			...
			buttons: [
				{
					...
					title: documentTitle,
					exportOptions: {
						columns: [1, 2, 3, 4], // Exclude columns with class "actions"
					}
				},
				{
					extend: 'excelHtml5',
	...
	var handleStatusRows = () => {
		...
		statusButtons.forEach(d => {
			...
			d.addEventListener('click', function (e) {
				...
				const status = parent.querySelectorAll('td')[3].querySelectorAll('a')[0].getAttribute('data-status');
				const url = parent.querySelectorAll('td')[3].querySelectorAll('a')[0].getAttribute('data-url');
